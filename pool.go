...
func NewPool(ctx context.Context, db *Client, limit uint64) *Pool {
	return &Pool{
		ctx:      ctx,
		dynamoDB: db,
		limit:    limit,
...
//Do executes a given Executable
func (p *Pool) Do(exe Executable) *Promise {
	if err := p.claimSlot(); err != nil {
		promise := NewPromise(context.Background())
		promise.SetResponse(nil, err)
		return promise
	}
	promise := NewPromise(p.ctx)
	go func() {
		defer p.subActive(1)
		execute(p.ctx, p.dynamoDB.DynamoDBClient(), exe, promise)
	}()
	return promise
}
